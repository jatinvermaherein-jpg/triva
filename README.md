...
| `404 (error code: 10062): Unknown interaction` from a **modal** (`SeasonCreateModal.on_submit`, `AnswerModal.on_submit`) | the modal answered only after its database work: a 4-week season is **43 statements**, ~120 round trips once the savepoint pairs inside `with conn:` are counted. Free on SQLite, seconds on Supabase | fixed: both modals `defer()` before the first statement and answer through `ui.reply`, so the confirmation edits the deferred placeholder instead of racing the 3-second window. **The season was still created and the answer was still saved** - press `👀 Season preview`, or reopen the night, rather than running it again |

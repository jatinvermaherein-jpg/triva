...
def main() -> int:
    ...
    # ------------------------------------------------- the int4 snowflake crash
    # Postgres INTEGER is 32-bit (max 2,147,483,647); SQLite's is 64-bit and a
    # Discord snowflake is ~1.5e18. The schema therefore created fine on Supabase
    # and died on the first real id - /setup's audit row - with
    # `psycopg.errors.NumericValueOutOfRange: integer out of range`. Every suite
    # passed because every fake id was small. Guards, one per layer:
    import re as _re
    _pg = D._pg_schema(D.SCHEMA)
    check("fresh Postgres tables carry no 32-bit INTEGER column",
          _re.search(r"\bINTEGER\b", _pg) is None, str(_pg.count("INTEGER")))
    check("the id identity clause survives the widening",
          "GENERATED BY DEFAULT AS IDENTITY" in _pg)
    check("migrated columns widen too (ADD COLUMN on Postgres)",
          D._pg_decl("INTEGER NOT NULL DEFAULT 0") == "BIGINT NOT NULL DEFAULT 0",
          D._pg_decl("INTEGER NOT NULL DEFAULT 0"))
    _SNOW = 1541282042128367626          # the magnitude that crashed the deploy
    _mem = D.connect(":memory:")
    D.audit(_mem, "deploy.probe", _SNOW, None, None, {"k": 1})
    _got = _mem.execute("SELECT actor_id FROM audit WHERE action='deploy.probe'").fetchone()
    check("a Discord-sized snowflake round-trips through audit",
          _got and _got["actor_id"] == _SNOW, str(dict(_got) if _got else None))
    # ...and the boot repair for databases created before the widening existed:
    # CREATE TABLE IF NOT EXISTS never revisits them, so connect() must.
    _wdb = _FakeWidenDb([{"table_name": "audit", "column_name": "actor_id"},
                         {"table_name": "someone_else", "column_name": "n"}])
    _widened = D._pg_widen_ints(_wdb)
    check("boot repair widens owned int4 columns", _widened == ["audit.actor_id"],
          str(_widened))
    check("boot repair leaves strangers' tables alone",
          len(_wdb.alters) == 1 and "someone_else" not in _wdb.alters[0],
          str(_wdb.alters))
    check("boot repair refreshes the type cache", _wdb.bootstrapped == 1)
    ...
    if url.startswith("postgres"):
        ...
        check("no int4 column survives the boot repair on the live server",
              not _narrow,
              str([(r["table_name"], r["column_name"]) for r in _narrow]))
        live.execute("INSERT INTO audit(actor_id,action,target,before,after,ts) "
                     "VALUES(?,?,?,?,?,?)",
                     (1541282042128367626, "deploy.probe", None, None, None, D.now()))
        _snow = live.execute(
            "SELECT actor_id FROM audit WHERE action='deploy.probe'").fetchone()["actor_id"]
        check("a Discord-sized snowflake round-trips through audit on Postgres",
              _snow == 1541282042128367626, str(_snow))
...
class _FakeWidenDb:
    """The least a PgConnection can be for _pg_widen_ints: a canned catalog, a
    recorder for the ALTERs, and a counter for the cache refresh."""

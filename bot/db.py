...
# Table names SCHEMA owns, parsed once. The Postgres boot repair (_pg_widen_ints)
# only touches these: a Supabase project can host other apps' tables, and
# rewriting a stranger's column types on our boot is not our business.
_SCHEMA_TABLES = frozenset(re.findall(r"CREATE TABLE IF NOT EXISTS (\w+)", SCHEMA))
...
def _migrate(db) -> list[str]:
    ...
    for table in {t for t, _, _ in MIGRATIONS}:
        ...
        for name, decl in migrations_for(table):
            if name not in have:
                if pg:
                    decl = _pg_decl(decl)   # int4 on Postgres cannot hold a snowflake
...
def connect(target) -> sqlite3.Connection:
    ...
    if conninfo:
        ...
        _migrate(db)
        _pg_widen_ints(db)          # pre-widen databases still carry int4; see above
        db.sync_sequences()
        return db
    path = str(target)
...
def _pg_schema(script: str) -> str:
    ...
    Two rewrites, because the engines disagree twice. `id INTEGER PRIMARY KEY` needs the
    identity clause (in Postgres it is otherwise just int+unique, with no auto-assign).
    And every other INTEGER becomes BIGINT: SQLite's INTEGER is 64-bit, but Postgres'
    INTEGER is 32-bit (max 2,147,483,647) while a Discord snowflake is ~1.5e18 - so a
    table created verbatim accepts the schema and then dies on the first real id with
    `integer out of range`. That is exactly how /setup's audit row crashed the deploy.
    Widening the small columns too (points, ordinals) costs nothing and keeps one rule.
    """
    out = re.sub(r"\bid\s+INTEGER\s+PRIMARY\s+KEY\b",
                 "id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY", script, flags=re.I)
    return re.sub(r"\bINTEGER\b", "BIGINT", out, flags=re.I)


def _pg_decl(decl: str) -> str:
    """A MIGRATIONS column declaration, spelled for Postgres.

    Same widening as _pg_schema: without it a migrated column (posted_by, points_in,
    ...) lands as int4 on Postgres while SCHEMA-built columns are int8, and the next
    snowflake written to it raises `integer out of range` mid-evening.
    """
    return re.sub(r"\bINTEGER\b", "BIGINT", decl, flags=re.I)


def _pg_widen_ints(db) -> list[str]:
    """ALTER every int4 column this schema owns to BIGINT. Runs on every PG boot.

    Why a repair and not just a schema fix: `CREATE TABLE IF NOT EXISTS` never touches
    an existing table, so every database created before _pg_schema widened is still
    int4 everywhere - and the failure is silent until a real Discord id is written,
    which is a grading run or a /setup, never a test with small fake ids. int4->int8
    is a lossless widening (no USING clause needed), defaults and indexes survive it,
    and Postgres allows the FKs to span the mixed types while the columns convert one
    by one. Returns the columns changed, so the boot log can say what it did.
    """
    rows = db._raw(
    ...
    for r in rows:
        ...
        if table not in _SCHEMA_TABLES:
            continue                     # another app's table: not ours to rewrite
        db._raw('ALTER TABLE "' + table + '" ALTER COLUMN "' + col + '" TYPE BIGINT')
        widened.append(table + "." + col)
    if widened:
        db._bootstrap()                  # the type cache still says int4; refresh it
    return widened
    ...
    def _coerce(self, sql: str, table, cols, params):
        ...
        for i, v in enumerate(out):
            ...
            if isinstance(v, bool) and "bool" not in want:
                out[i] = 1 if v else 0
            elif isinstance(v, float) and want.split("(")[0] in (
                    "smallint", "integer", "bigint", "numeric", "decimal") \
                    and v.is_integer():
                # "bigint" does NOT start with "int", so this is an explicit set rather
                # than a prefix test: every integer column widened to int8 must still
                # accept a whole-valued float the way SQLite does.
                out[i] = int(v)
        return tuple(out)

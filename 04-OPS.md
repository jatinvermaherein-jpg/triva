...
| `/setup mode:SETUP` says **"the application did not respond"**, creates nothing, and Railway's log is **empty** | Four separate faults, all fixed — see README "The /setup dead-ends". In short: the picker was `stop()`ed on any re-run (a stopped view makes discord.py discard the click *before* the callback), staff checks matched the role **name** instead of the stored id, the owner bypass compared you against the **bot's** id so the first setup was unconfirmable, and stdout was block-buffered so Railway showed nothing | Deploy this commit. If it recurs, `/setup mode:STATUS` still reports the wiring, and the log now names the failing control |
| Railway's deploy log is blank while the bot is clearly running | Python block-buffers a pipe in 8 KB chunks; a quiet bot fills that in hours | Fixed: stdout is line-buffered at boot and logging writes to stdout. A blank log now means a blank log |
| **Only `/setup` and `/setup_panel` appear in Discord** — no `/season-create`, no `/question-add`, nothing else | The other 12 commands were indented one level too deep, landing *inside* `_setup_status()` **after its `return`** — dead code that never registered. The test suite walked the AST, which descends into nested scopes, so it saw them and reported green | Fixed by moving `_setup_status` below `build_tree`. `tests/test_deploy.py` now builds the real tree and asserts all 13 names, so this cannot regress |
| `403 Forbidden (50001): Missing Access` on `/setup-panel`, `/pin-board` or `/pin-checkout` | The bot lacks **View Channel**, **Send Messages** or **Embed Links** in the target channel — very often from a **category** override above it, or the bot's role sitting below one that denies View Channel | The bot now names the exact missing permission *before* writing anything, instead of a traceback. Fix it in that channel's Edit Channel → Permissions |
